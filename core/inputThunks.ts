...
export const handleInput = createAsyncThunk<
    { inputType: string; inputData: any; context: string },
    HandleInputPayload,
    { state: RootState }
>(
    'input/handleInput',
        ...
        switch (context) {
            case 'world':
                if (inputType === 'direction') {
                    dispatch(movePlayer({ direction: inputData.direction as Direction }));
            ...
            case 'gambling':
                if (inputType === 'direction') {
                    dispatch(navigateMenu({ direction: inputData.direction as Direction }));
                } else if (inputType === 'action') {
...
>(
    ...
    async (_, { dispatch, getState }) => {
        return await dispatch(handleInput({ inputType: 'action' })).unwrap();
    }
);
...
>(
    ...
    async ({ direction }, { dispatch, getState }) => {
        return await dispatch(handleInput({ 
            inputType: 'direction', 
            inputData: { direction } 
        })).unwrap();
    }
...
>(
    'input/handleCancel',
    async (_, { dispatch, getState }) => {
        return await dispatch(handleInput({ inputType: 'cancel' })).unwrap();
    }
);
